...
static void init_report(MigrationReport* report) {
    ...
    memset(report->target_os, 0, sizeof(report->target_os));
    memset(report->target_shell, 0, sizeof(report->target_shell));
...
bool migrate_script(const char* filepath, const char* target_os, const char* target_shell, const char* dict_path, MigrationReport* out_report) {
    ...
    if (target_os) strncpy(out_report->target_os, target_os, sizeof(out_report->target_os) - 1);
    if (target_shell) strncpy(out_report->target_shell, target_shell, sizeof(out_report->target_shell) - 1);
...
bool write_migrated_script(const MigrationReport* report, const char* output_path) {
    ...
    bool in_preserved_block = false;
    
    for (int i = 0; i < report->entry_count; i++) {
        const MigrationEntry* entry = &report->entries[i];
        
        bool is_preserved_type = (entry->classification == LINE_CONTROL_FLOW || 
                                  entry->classification == LINE_VARIABLE || 
                                  entry->classification == LINE_PIPELINE);
                                  
        if (!is_preserved_type && in_preserved_block) {
            fprintf(file, "# End preserved block\n");
            in_preserved_block = false;
        }
        
        if (entry->classification == LINE_SHEBANG) {
            fprintf(file, "# Generated by CmdBridge\n");
            fprintf(file, "# Target OS: %s\n", report->target_os[0] ? report->target_os : "Unknown");
            fprintf(file, "# Target Shell: %s\n", report->target_shell[0] ? report->target_shell : "Unknown");
        } else if (entry->classification == LINE_COMMAND) {
            if (entry->status == STATUS_TRANSLATED) {
                ...
                fprintf(file, "# CmdBridge Notice: Approximate Translation\n");
                ...
                fprintf(file, "# CmdBridge Notice: Unsupported Command\n");
                fprintf(file, "# Reason: %s\n", reason_code_to_string(entry->reason));
                fprintf(file, "# Original: %s\n", entry->original_line);
            }
        } else if (is_preserved_type) {
            if (!in_preserved_block) {
                fprintf(file, "# CmdBridge Notice\n");
                fprintf(file, "# Unsupported block preserved below.\n");
...
void print_migration_summary(const MigrationReport* report) {
    ...
    if (report->translated > 0 || report->approximate > 0 || report->unsupported > 0 || report->preserved > 0) {
        printf("\nDetailed Report:\n");
        printf("-----------------\n");
        
        if (report->approximate > 0) {
            printf("\nApproximate Translations:\n");
            for (int i = 0; i < report->entry_count; i++) {
                if (report->entries[i].status == STATUS_APPROXIMATE) {
                    printf("  Line %d: %s\n", report->entries[i].line_number, report->entries[i].original_line);
                    printf("  Reason:  %s\n\n", reason_code_to_string(report->entries[i].reason));
                }
            }
        }
        ...
        if (report->unsupported > 0) {
            ...
            for (int i = 0; i < report->entry_count; i++) {
                if (report->entries[i].status == STATUS_UNSUPPORTED) {
                    printf("  Line %d: %s\n", report->entries[i].line_number, report->entries[i].original_line);
                    printf("  Reason:  %s\n\n", reason_code_to_string(report->entries[i].reason));
                }
            }
